...
class PreprocessText:
    ...
    def preprocess_dataset(self, df, column):
        """Nettoyer toute la colonne du DataFrame."""
        if df is None or column is None:
            raise ValueError("Le DataFrame et la colonne doivent être fournis pour cette méthode.")
        
        return self.preprocess_text_for_dataset(df, column)  # Nettoyage en batch

    # Fonction pour obtenir la moyenne des vecteurs FastText pour chaque texte
    ...
    def predict_text(self, df, image_path, text_model_weights,model):
        ...
        text_input = self.preprocess_text_for_dataset(df, "description", image_path=image_path)
        ...
        self.text_model = tf.keras.models.load_model(text_model_weights)
        pred_im = self.text_model.predict(text_embedded_input)
        predicted_class_index_txt = np.argmax(pred_im[0])
...
predicted_category, cleaned_text = preprocessor.predict_text(df, image_path_input, TEXT_MODEL_PATH,word2vec_model)
